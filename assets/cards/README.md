...
Daily reports that cover multiple predicted matches on the same China-time date may include an additional overview card named `daily-YYYY-MM-DD-summary.png`. This card summarizes the same-day schedule, win/draw/loss probabilities, and three scoreline scenarios with one rationale per scenario. It supplements per-match images and does not replace them.
