...
[English](platform-copy.md) | [简体中文](platform-copy.zh-CN.md) | [Methodology](methodology.md)

This page stores standing account-introduction copy for World Cup publishing. Use it alongside each match-specific prediction post.

## Copy Rules

- Say clearly that the content is a football match prediction.
- Say that the account uses ChatGPT's highest reasoning model for World Cup match analysis.
- Mention that match forecasts cover win/draw/loss, likely scorelines, and key risk factors.
- Do not present any prediction as guaranteed.
- Include an investment-advice disclaimer where the platform format allows it.

## Douyin / TikTok-Style Short Video Caption

During the 2026 World Cup, this account will use ChatGPT's highest reasoning model to publish match-by-match football predictions: win/draw/loss lean, projected scoreline, tactical keys, lineup and injury context, weather and schedule factors, market movement, expert views, and major risks.

This is a match prediction only and does not constitute investment advice.

## Xiaohongshu / Lifestyle Note Caption
